 ...
 * nodes within a binary search tree. You can use this class' insert method to
 * build a binary search tree, and its toString method to display the level
...
public class RedBlackTree<T extends Comparable<T>> {
	...
	public void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
		...
		// will perform a right rotation
		if (child.isLeftChild()) {

			// Putting child in the right spot
			if (grandparent == null)
				root = child;
			...
			// Putting parent in the right spot
	...
	/**
	 * This private method enforces the RBT properties after a new node has been
	 * added.
	 * 
	 * It only works with red nodes that are children of red nodes because all new
	 * nodes are red, and if their parent is black, then no property is violated.
	 * 
	 * @param child - new node that has just been added
	 */
	private void enforceRBTreePropertiesAfterInsert(Node<T> child) {
		// This method only takes red nodes
		if (child == null || child.isBlack)
			return;

		Node<T> parent = child.parent;

		// This method only works with red nodes that are children of red nodes
		if (parent == null || parent.isBlack)
			return;

		Node<T> grandParent = parent.parent;
		Node<T> siblingOfParent = parent.isLeftChild() ? grandParent.rightChild : grandParent.leftChild;

		// Case 1 - Parent's sibling is black or null
		if (siblingOfParent == null || siblingOfParent.isBlack)
			// If the child and its parent are both left children or both right children,
			if ((child.isLeftChild() && parent.isLeftChild()) || (!child.isLeftChild() && !parent.isLeftChild())) {

				// we perform a single right or left rotation and then recolour.
				...
				// Otherwise we perform a double rotation (right-left or left-right) and then
				// recolour.
				this.rotate(child, parent);
				this.rotate(child, grandParent);
				child.isBlack = true;
				grandParent.isBlack = false;
		...
		// Case 2 - Parent's sibling is red
			...
			// We recolour to restore properties and call this method recursively to ensure
			// the red properties aren't violated higher up
			parent.isBlack = true;
			siblingOfParent.isBlack = true;
